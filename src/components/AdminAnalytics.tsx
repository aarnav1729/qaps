...
const AdminAnalytics: React.FC<AdminAnalyticsProps> = ({ qapData }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Turnaround Time Analytics</h2>
        <p className="text-gray-600">Average processing times by plant, level, and user</p>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="text-sm font-medium text-gray-600 mb-2 block">Plant</label>
            ...
            <div>
              <label className="text-sm font-medium text-gray-600 mb-2 block">User</label>
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData}>
                ...
                <Tooltip formatter={(value, name) => [`${value} hours`, 'Average Time']} />
                <Legend />
            ...
            <table className="w-full border-collapse border border-gray-300">
              <thead>
                <tr className="bg-gray-100">
                  ...
                  <th className="border border-gray-300 p-3 text-left">Level/Role</th>
                  <th className="border border-gray-300 p-3 text-left">User</th>
                  <th className="border border-gray-300 p-3 text-right">Avg Time</th>
                  ...
                  analytics.map((item, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      <td className="border border-gray-300 p-3">
                        <Badge variant="outline">{item.plant.toUpperCase()}</Badge>
                      </td>
                      <td className="border border-gray-300 p-3">
                        <Badge 
                          variant={item.role === 'Overall' ? 'default' : 'secondary'}
                        >
                          {item.role}
                        </Badge>
                      </td>
                      <td className="border border-gray-300 p-3">{item.user}</td>
